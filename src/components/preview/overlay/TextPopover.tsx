...
//   - the three hero lines are plain strings, so the card holds a TEXTAREA.
//     Enter saves, Shift+Enter is a new line, Escape cancels.
//   - the seven curated support lines are rich twins: bold and italic, nothing
//     else. The card holds a CONTENTEDITABLE with two buttons. A twin is ONE
//     line on this site (InlineRich.astro emits no breaks), so Enter saves and
//     there is no second line to make.
//
...
// string: the twin is seeded from that string, so an editor's first bold keeps
...
//
// STEGA. Everything read here comes from the DRAFT DOCUMENT over the comlink,
// which carries no stega, and `resolveTextTarget` cleans it anyway. Nothing this
// card writes can therefore carry markers into the dataset. The rendered page
// keeps its click-to-edit because the swap back into the DOM is
// useInstantText's job, and that path reattaches each text node's own run
...
import { TOOL, bar, button, caption, card, field, primaryButton } from './styles.ts';

/** Tags whose boundary is a word boundary in a value that keeps no lines. */
...
function domToRuns(root: Node): InlineRun[] {
  ...
  const walk = (node: Node, strong: boolean, em: boolean) => {
    ...
    if (tag === 'br') {
      runs.push({ text: ' ', strong: false, em: false });
      return;
    }
    if (BLOCKISH.has(tag) && runs.length) runs.push({ text: ' ', strong: false, em: false });
          ...
          >
            ...
            {target.kind === 'plain' ? (
              ...
              <div
                ...
                style={field}
                onKeyDown={(event) => {
                  // Enter saves. A twin holds one line, so there is no
                  // Shift+Enter case to make room for.
                  if (event.key === 'Enter') {
                    event.preventDefault();
                    save();
                  }
                ...
                onPaste={(event) => {
                  // Bold and italic. Anything else in the clipboard would be
                  // thrown away on save, so throw it away on the way IN and let
                  // the editor see what they are going to get. What survives is
                  // exactly what the allow-list parser keeps.
                  event.preventDefault();
