 ...
 * optimistic write from inside the frame would arrive as `rebased.local` (this
 * template makes none today; the library of record's in-canvas controls do), a
...
export function useInstantText(pageId: string, onDocument?: () => void): void {
  ...
  //
  // AND IT MUST BE THIS PAGE'S DOCUMENT (church-starter, 2026-08-28). This is
  // the one place this file diverges from the library of record's copy, and the
  // reason is the mount point at the other end. There, LiveDraftBridge hangs off
  // a preview navigator that has already resolved WHICH page the preview is
  // showing, so only that page's draft is ever posted. This template has no
  // navigator (its equivalents are Studio document actions), so the bridge is
  // mounted from the document form itself and posts whatever the editor has
  // open — Site settings, a sermon, another page. Without this check the first
  // such snapshot would become `lastSeen`, and the NEXT real edit to this page
  // would be diffed against a different document entirely: every shared field
  // name (`title`, `heading`, `body`) would read as a change, and any that
  // matched a text node exactly would write another document's words onto this
  // page. The guard makes the local channel say only what the actor path
  // already says — this page, or nothing. See src/sanity/components/
  // LiveDraftBridge.tsx for the mounting decision.
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      if (event.origin !== window.location.origin) return;
      const message = parseLiveDraft(event.data);
      if (!message?.document) return;
      const id = message.document._id;
      if (id !== pageId && id !== `drafts.${pageId}`) return;
      lastLocalAt.current = Date.now();
      applyDocument(message.document, 'local');
    };
    window.addEventListener('message', onMessage);
    return () => window.removeEventListener('message', onMessage);
  }, [applyDocument, pageId]);
  ...
  useEffect(() => {
    ...
    const subscriptions = ACTOR_EVENTS.map((type) =>
      actor.on(type, (event: ActorEvent) => {
        ...
        // staff, ministries, events, sermons. Only this page's own document is
        // diffed; a change to a shared document reaches the page through the
        // soft refresh, because its stega names ITS id, not this one.
