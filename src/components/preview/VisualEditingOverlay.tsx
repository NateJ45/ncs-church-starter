...
// re-render. (Ported from ncs-astro-sanity-starter 2026-08-28.)
//
...
//    src/lib/preview-navigation.ts models that retry as a pure state machine.
//    It is UNUSED in this template today: the retry has to be driven from a
//    Studio-side panel watching `params.preview`, and this Studio has no
//    navigator panel, so the file ships tested and ready rather than wired.
...
//     is that replacing <main> re-decoded every image and blocked the main
...
export default function VisualEditingOverlay({ pageId }: Props) {
  ...
  const refresh = useCallback(
    (payload: HistoryRefresh): false | Promise<void> => {
      ...
      // when a shared or referenced doc changed (staff, ministries, events,
      // sermons, FAQ items, site settings: anything that is not some OTHER
      // custom page), since those can appear on this page.
