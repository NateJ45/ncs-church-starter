...
When you change a Sanity schema (`src/sanity/schemaTypes/**`), run `npm run typegen` and commit the regenerated `src/lib/sanity.types.ts` and `schema.json`. There is no separate Studio deploy to remember: since 2026-08-28 the Studio is embedded at `/studio` and ships with the site build, so the schema an editor sees is always the schema that was deployed. What DOES need doing by hand is opening `/studio` in a browser afterwards, because schema errors are fatal at browser runtime while passing the build.
